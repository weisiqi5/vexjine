...
public:
	...
	void waitToBeServiced() {
		if(AgentTester::virtualExecution) {
			threadEventsBehaviour->onRequestingLock(&lock);
		}
		...
		while (!serviced) {
			if(AgentTester::virtualExecution) {
				...
				if (threadEventsBehaviour->beforeReleasingLockOfAnyReplacedWaiting(&lock, true)) {

					pthread_mutex_unlock(&lock);
					threadEventsBehaviour->onReplacedTimedWaiting(&lock, 0, 0, true);	// stay blocked in here until the virtual timeout expires
					pthread_mutex_lock(&lock);
				}
			} else {
	...
	void setServiced() {
		...
		if(AgentTester::virtualExecution) {
			//int monitorId = 0; memcpy(&monitorId, this, sizeof(int));
			threadEventsBehaviour->onSignallingOnObject(&lock);
		}
		pthread_cond_signal(&cond);

		if(AgentTester::virtualExecution) {
			//int monitorId = 0; memcpy(&monitorId, this, sizeof(int));
			threadEventsBehaviour->onReleasingLock(&lock);
		}
		pthread_mutex_unlock(&lock);
	}
...
//		int thinkingTimeNanoSeconds = (int)((nextThinkTime - thinkingTimeMilliSeconds) * 1000000);
		...
		if(AgentTester::virtualExecution) {
			...
			long thinkingTimeMilliSeconds = (long)nextThinkTime;
			int thinkingTimeNanoSeconds = (int)((nextThinkTime - thinkingTimeMilliSeconds) * 1000000);
			threadEventsBehaviour->onSleep(thinkingTimeMilliSeconds, thinkingTimeNanoSeconds);
		} else {
			nanosleep(&delay, &rem);
		}
