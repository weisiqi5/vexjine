...
/**
 * XXX Stores and manipulates various related timers associated with a thread.
 */
class Timers {
 public:
  /**
  ...
  void setBlockingTimeToLastResumedTime() {
    std::swap(estimatedRealTime, resumedLastAt);
  }

  void unsetBlockingTimeToLastResumedTime() {
    std::swap(estimatedRealTime, resumedLastAt);
  }
  ...
  /**
   * Using a heuristic based on the virtual time difference between the
   * last possible running thread (\p minimumMaybeAliveTime) and the real
   * time expired.
   *
   * If #timeout is <= 0 or if the difference between #estimatedRealTime and
   * \p minimumMaybeAliveTime is < 0 then return true.
   *
   * Else if the difference between Time#getRealTime and #lastRealTime is
   * greater than \p remainingTimeToExpireTimeout then return true; otherwise
   * return false.
   */
  ...
  /**
   * Update #estimatedRealTime to \p time if \p time is larger.
   */
