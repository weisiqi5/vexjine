...
class DisablingMulticoreVirtualTimeline : public MulticoreVirtualTimeline
{
 public:
   ...
   */
  long long const& getUpdatedGlobalTime();
...
/**
 * XXX Stores a snapshot of global and local virtual times.
 */
class VirtualTimeSnapshot
{
 public:
  /**
   * Create a new VirtualTimeSnapshot with default, nil values.
   */
  VirtualTimeSnapshot() {
    globalTime = 0;
    localTimelineId = 0;
  ...
  /**
   * Set #globalTime and #localTime to \p _globalTime, and #localTimelineId to
   * 0.
   */
  ...
  /**
   * Set #globalTime, #localTime and #localTimelineId to \p _globalTime, \p
   * _localTime and \p _localTimelineId respectively.
   */
  ...
  /**
   * Subtract \p globalTime and \p localTime from VirtualTimeSnapshot \p rhs
   * from this VirtualTimeSnapshot's #globalTime and #localTime, if their
   * #localTimelineId match.
   */
  VirtualTimeSnapshot& operator-=(const VirtualTimeSnapshot &rhs);

 private:
  /**
   * Stored global virtual time.
   */
  long long globalTime;

  /**
   * Stored local virtual time for thread with timeline ID #localTimelineId.
   */
  long long localTime;

  /**
   * Timeline ID for #localTime.
   */
  int localTimelineId;
};
...
/**
 * XXX Interface for manipulating a VirtualTimeline using a Timers associated
 * with a VexThreadState.
 */
...
/**
 * XXX Implementation of VirtualTimelineController that logs the
 * VirtualTimeSnapshot before and after changes to the timeline using a
 * TimeLoggingBehaviour object.
 *
 * It passes all calls to #controller, which is the actual
 * VirtualTimelineController; StatisticsEnabledVirtualTimelineController is
